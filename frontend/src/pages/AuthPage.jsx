    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 440, zIndex: 1 }} className="animate-scaleIn">
        <div className="text-center mb-24">
          <div style={{ fontFamily: 'var(--font-heading)', fontSize: '2.5rem', fontWeight: 800, color: '#ffffff', letterSpacing: '-0.03em', marginBottom: 4 }}>
            FestTracker
          </div>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="form-group">
        ...
        <input className={`form-input${errors.username ? ' error' : ''}`} type="email"
          placeholder="name@company.com" {...register('username', { required: 'Email is required' })} />
        {errors.username && <span className="form-error">{errors.username.message}</span>}
      </div>
      <div className="form-group">
        <label className="form-label">Password</label>
        <input className={`form-input${errors.password ? ' error' : ''}`} type="password"
          placeholder="Enter password" {...register('password', { required: 'Password is required' })} />
  ...
  if (step === 2) return (
    <form onSubmit={hs2(onVerify)} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="text-center" style={{ padding: '12px 0' }}>
        <h3 style={{ marginBottom: 4 }}>Verify Email Address</h3>
        <p style={{ fontSize: '0.875rem' }}>Enter the 6-digit code sent to <strong>{pendingUsername}</strong></p>
      </div>
      <div className="form-group">
  ...
  return (
    <form onSubmit={handleSubmit(onRegister)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {['USER', 'ORGANISER'].map(r => (
            <label key={r} style={{
              ...
              border: `1px solid ${watch('role') === r ? 'var(--primary)' : 'var(--border)'}`,
              borderRadius: 'var(--radius-md)', flex: 1, justifyContent: 'center', fontWeight: 600,
              fontSize: '0.875rem',
              color: watch('role') === r ? 'var(--primary)' : 'var(--text-secondary)',
              background: watch('role') === r ? '#e0e7ff' : 'var(--bg-white)',
      ...
      <div className="form-group">
        <label className="form-label">Email Address</label>
        <input className={`form-input${errors.username ? ' error' : ''}`} type="email"
          placeholder="name@company.com" {...register('username', { required: 'Email required' })} />
        {errors.username && <span className="form-error">{errors.username.message}</span>}
      </div>
      <div className="grid-2">
        <div className="form-group">
          <label className="form-label">Password</label>
          <input className={`form-input${errors.password ? ' error' : ''}`} type="password"
            placeholder="Min 8 chars" {...register('password', { required: true, minLength: { value: 8, message: 'Min 8 characters' } })} />
          {errors.password && <span className="form-error">{errors.password.message}</span>}
        </div>
        <div className="form-group">
...
function ForgotForm({ onSuccess }) {
  ...
  return (
    <form onSubmit={hs1(onSend)} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <p style={{ fontSize: '0.875rem' }}>Enter your registered email address to receive a password reset verification code.</p>
      <div className="form-group">
        <label className="form-label">Email Address</label>
        <input className="form-input" type="email" placeholder="name@company.com" {...r1('username', { required: true })} />
      </div>
